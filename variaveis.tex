...
\program{PROFMAT –- Mestrado Profissional em Matemática em Rede Nacional}
\programen{PROFMAT –- Professional Master's Degree in Mathematics in a National Network}
\degree{Mestre}
\degreearea{Matemática}
% Caso seja TSI:
...
\author{GERSON CESAR GROBE DE MIRANDA}  % Seu nome
\authorbib{Miranda, Gerson Cesar Grobe de} % Seu nome para referência bibliográfica (Sobrenome, Nome)
\title{INTRODUÇÃO DA DISSERTAÇÃO DO PROFMAT} % Título do trabalho

\titleFA{INTRODUÇÃO DA DISSERTAÇÃO DO PROFMAT}

\titleen{Introduction do Profmat's Dissertation} % Título traduzido para inglês
\advisor{Prof. Dr. Rudimar Luiz Nós.} % Nome do orientador. Lembre-se de prefixar com "Prof. Dr.", "Profª. Drª.", "Prof. Me." ou "Profª. Me."}
...
\universityunit{Departamento Acadêmico de Matemática}
\address{CURITIBA}
\addressen{Campus Curitiba, PR, Brazil}
\documenttype{Dissertação}
\documenttypeen{Dissertation}
\degreetype{Mestrado}
